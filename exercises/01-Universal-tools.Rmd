...
These are the class notes (Part 1 of 3) for *R for Big Data*, a workshop taught at *Strata + Hadoop World 2016 NYC*.
...
**Exercise 2**: *Write a definition in your own words for each of the dplyr functions below. Stop when you get to Exercise 3*.
...
**Exercise 3**: *The `iris` data set contains biometric measurements of three species of flower. In the code chunk below, use dplyr functions to return just the Species name, mean Petal.Width and mean Petal.Length of the Species with the largest difference between mean Petal.Width and mean Petal.Length.*
...
**Exercise 4**: *You can use ` %>% ` to string many functions together into a long pipe. Use ` %>% ` to turn the code below (from Exercise 3) into a single long pipe.
...
**Mutating joins** add new variables to one data set from matching rows in a second data set. dplyr has 4 dmutating joins. The joins differ in the information they do not return.

* `left_join` - Drops rows from the *second* data set that do not match rows in the first.

* `right_join` - Drops rows from the *first* data set that do not match rows in the second.

* `inner_join` - Drops rows from *both* data sets that do not match rows in the other data set.

* `full_join` - Drops rows from *neither* data set (retains all values).
...
# Which airline has the oldest planes? (relies on distinct)
flights %>%
