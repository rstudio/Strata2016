...
2. translate R code to SQL `SELECT` statements to run in the DBMS
...
flights_tbl <- tbl(air, "flights")
airlines_tbl <- tbl(air, "carriers")

# Calculate average speed
avg_speed_tbl <- flights_tbl %>%
  filter(actualelapsedtime > 30) %>%
  mutate(mph = distance / actualelapsedtime * 60) %>%
  group_by(uniquecarrier) %>%
  summarise(avg_mph = mean(mph), avg_dist = mean(distance), count = n()) %>%
  left_join(airlines_tbl, by = c("uniquecarrier" = "code")) %>%
  mutate(name = substring(description, 1L, 32L)) %>%
  select(name, avg_mph, avg_dist, count) %>%
  arrange(desc(avg_mph))

# Inspect SQL
sql_render(avg_speed_tbl)

# Collect into R for visualization
avg_speed <- avg_speed_tbl %>%
  collect %>%
  print

# Create interactive plot
figure(width = NULL, height = NULL, title = "", legend_location = NULL) %>%
  ly_points(avg_dist, avg_mph, data = avg_speed,
    size = rank(count), color = count,
    hover = list(name, avg_dist, avg_mph, count))
